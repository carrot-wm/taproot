...
/// Initialize origin's runtime state when origin is linked into a shared
/// library whose program entry point never runs.
///
/// The program's own runtime owns the main thread; this only prepares this
/// copy of origin to create new threads that carry the executable's TLS
/// layout. Idempotent.
///
/// # Safety
///
/// `envp` must point to a null-terminated environment block followed in
/// memory by the kernel-format auxv array. A fabricated block with an empty
/// environment is fine.
pub unsafe fn init_from_dso(envp: *mut *mut u8) {
    #[cfg(feature = "param")]
    unsafe {
        rustix::param::init(envp);
    }
