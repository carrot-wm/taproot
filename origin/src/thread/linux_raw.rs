    ...
    unsafe {
        ...
        while current_phdr != phdrs_end {
            ...
            match phdr.p_type {
                ...
                // we get.
                PT_PHDR => phdr_offset = Some(first_phdr.addr().wrapping_sub(phdr.p_vaddr)),
                PT_DYNAMIC => {
                    dynamic_offset = Some(dynamic_addr.addr().wrapping_sub(phdr.p_vaddr))
                }
        ...
        // Prefer the `PT_PHDR`-derived offset: comparing `_DYNAMIC` against
        // the `PT_DYNAMIC` header is only meaningful when this code is
        // linked into the executable whose phdrs these are; in a shared
        // library, `_DYNAMIC` is the library's own and the difference is
        // meaningless.
        let offset = phdr_offset.or(dynamic_offset).unwrap_or(0);
